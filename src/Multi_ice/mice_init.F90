...
subroutine ice_init
   ...
   &iself,nxq,zctr,dldxy,tr_nd,nvrt,ics
  ...
  !Check
    ...
    &ice_advection/=5.and.ice_advection/=6.and.ice_advection/=7.and.ice_advection/=8) call parallel_abort('ice_init: ice_advection')
    if(ice_therm_on/=0.and.ice_therm_on/=1) call parallel_abort('ice_init: ice_therm_on')
    if(ievp/=0.and.ievp/=1.and.ievp/=2.and.ievp/=3) call parallel_abort('ice_init: ievp')
    ...
    allocate(u_ice(npa),v_ice(npa),ice_tr(ntr_ice,npa),ice_tr0(ntr_ice,nea), &
     ...
     &stress_atmice_y(npa),fsrad_ice_out0(npa),sradiold(npa),netheatiold(npa),Tbu(npa),strength(npa),&
     sstiold(npa),floenum(npa),dudicex(6,npa),dudicey(6,npa),stat=istat)
     ...
     !  endif
       ...
       evaporation=0; fsrad_ice_out0=0;Tbu=0;sradiold=0;netheatiold=0;floenum=0
...
! Read before the MICE geographic-coordinate check in schism_init.
! Keep the default (no idealized_nml) behavior as a realistic case.
subroutine read_idealized_ice_config
  ...
  if(config_status/=0) call parallel_abort('ice_init: cannot open namelist.icepack')
...
end subroutine read_idealized_ice_config
