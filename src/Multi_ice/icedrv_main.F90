...
!--------- Declare all the variables used or required by Icepack
          ...
          real (kind=dbl_kind), allocatable, save :: & ! DIM nx,ncat
             m_uvel(:,:) , & ! x-component of velocity (m/s) on the nodes for each category
             m_vvel(:,:) , & ! y-component of velocity (m/s) on the nodes for each category
         ...
         real (kind=dbl_kind), public, dimension (:,:), allocatable :: &
          ...
          real (kind=dbl_kind), allocatable, & ! DIM nx,ncat
             ...
             sstn_old(:,:)   , & ! SST for each ice category and open water (old)
             hocnn_old(:,:)  , & ! net heat fulx of each ice category(old)
             hocnn(:,:)      , & ! net heat fulx of each ice category
             fswthrun_old(:,:),& ! SW through ice to ocean(old)     (W/m^2)
             fbotn(:,:)       ,& ! heat flux at bottom surface of ice (excluding excess) for each category (W/m^2)
             frzmltn(:,:)     ,& ! freezing/melting potential (W/m^2)for each category
             sstnbeta(:)      ,& ! mixing term for sstn
             sstnfsd_rad(:)   ,& ! mean radius of floe size
             nfloe(:)            ! number of floe
