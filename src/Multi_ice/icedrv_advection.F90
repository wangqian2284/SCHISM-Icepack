...
!#include "../associate_mesh.h"
    ...
    module subroutine init_advection_icepack
        ...
        allocate(flux_matrix00_m(mnei_p,nx,2,ncat)) 
        ...
        !Wet node
          ...
          do j=1,nne(i)
            ...
            !if(tmpnode==i) then
               tmpx2 = trc_o(tmpnode)
            !else
            !   tmpx2 = trc_o(tmpnode)*-1.d0
            !endif
            flux_matrix1(j,i,trc_n) = flux_matrix1(j,i,trc_d)*tmpx2
            flux_matrix2(j,i,trc_n) = flux_matrix2(j,i,trc_d)*tmpx2
            sumtmp = (flux_matrix1(j,i,trc_n)+flux_matrix2(j,i,trc_n))/2.d0
            !if(i.eq.tmpnode) then
            !   if(sumtmp<c0) write(12,*) 'negative vicen or vsonn',i,j,tmpnode,flux_matrix00(:,i,1),tmpx2,flux_matrix1(:,i,trc_d),flux_matrix2(:,i,trc_d)
            !endif
            nodeid = elnode(nxq(1,id,i34(ie)),ie)
            fluxchan1=fluxchan1+(flux_matrix1(j,i,trc_n)+flux_matrix2(j,i,trc_n))/2.d0
...
!            ytmp=xctr(ie)-xcj(isd2)
            ...
            sumtmp = (flux_matrix3(j,i,trc_n) + flux_matrix4(j,i,trc_n))/2.d0
            !if(i.eq.tmpnode) then
            !   if(sumtmp<c0) write(12,*) 'negative vicen or vsonn2',i,j,tmpnode,flux_matrix00(:,i,2),tmpx2,flux_matrix3(:,i,trc_d),flux_matrix4(:,i,trc_d)
            !endif
            nodeid = elnode(nxq(i34(ie)-1,id,i34(ie)),ie)
      ...
      do i = 1,nx_nh
       trc(i) = d_tr0(i) - trl0(i) !+ d_tr(i)
       !if(trc(i)<0) write(12,*) 'negative vicen or vsonn',trc_n,i,d_tr0(i),trc_o(i),trl0(i),flux_matrix00(:,i,1),&
       !&flux_matrix00(:,i,2),flux_matrix1(:,i,trc_d),flux_matrix2(:,i,trc_d),flux_matrix3(:,i,trc_d),flux_matrix4(:,i,trc_d)
      enddo
...
!=======================================================================
    ...
    subroutine tvdu_icepack_m(trc,muvel,mvvel,trc_n,catm,adv_threshold0)
        
      !use mod_mesh     
       use mice_module
       use schism_glbl, only: dldxy
      implicit none
      integer (kind=int_kind), intent(in) :: trc_n,catm
      real(kind=dbl_kind), dimension(nx), intent(inout) :: trc,adv_threshold0  
      real(kind=dbl_kind), dimension(nx), intent(in) :: muvel,mvvel
      real   (kind=dbl_kind)            :: suma,xctr2,yctr2,vnor1,vnor2,sumtmp,areatmp,&
                                           & tmpx2,tmpx3,tmpy2,tmpy3,utmp,vtmp,xtmp,ytmp,vnorm,&
                                           & trctmp, trcmin, trcmax ,puny ,tmp1 ,fluxchan0, tmp2, adv_threshold,&
                                           & tmp3, psi_r, sum1, sum2
    ...
    do ie=1,nx_elem
          ...
          do m=1,i34(ie) !side
            ...
            n2=isidenode(2,elside(m,ie))
            if(ics==1) then
               swild10(m,1) = 0.5*(muvel(n1)+muvel(n2))
               swild10(m,2) = 0.5*(mvvel(n1)+mvvel(n2))
               swild(m) = (trc(n1)+trc(n2))*0.5
               swild2(m,1) = muvel(elnode(m,ie))
               swild2(m,2) = mvvel(elnode(m,ie))
            else
               swild10(m,1) = 0.5*(muvel(n1)*dot_product(pframe(:,1,n1),eframe(:,1,ie))+mvvel(n1)*dot_product(pframe(:,2,n1),eframe(:,1,ie))&
                                 &+muvel(n2)*dot_product(pframe(:,1,n2),eframe(:,1,ie))+mvvel(n2)*dot_product(pframe(:,2,n2),eframe(:,1,ie)))
               swild10(m,2) = 0.5*(muvel(n1)*dot_product(pframe(:,1,n1),eframe(:,2,ie))+mvvel(n1)*dot_product(pframe(:,2,n1),eframe(:,2,ie))&
                                 &+muvel(n2)*dot_product(pframe(:,1,n2),eframe(:,2,ie))+mvvel(n2)*dot_product(pframe(:,2,n2),eframe(:,2,ie)))                      
               swild(m) = (trc(n1)+trc(n2))*0.5
               swild2(m,1) = muvel(elnode(m,ie))*dot_product(pframe(:,1,elnode(m,ie)),eframe(:,1,ie))+mvvel(elnode(m,ie))*dot_product(pframe(:,2,elnode(m,ie)),eframe(:,1,ie))
               swild2(m,2) = muvel(elnode(m,ie))*dot_product(pframe(:,1,elnode(m,ie)),eframe(:,2,ie))+mvvel(elnode(m,ie))*dot_product(pframe(:,2,elnode(m,ie)),eframe(:,2,ie))
            endif
            !swild10(m,1) = 0.5*(muvel(n1)*trc(n1)*dot_product(pframe(:,1,n1),sframe2(:,1,elside(m,ie)))+mvvel(n1)*trc(n1)*dot_product(pframe(:,2,n1),sframe2(:,1,elside(m,ie)))&
            !                 &+muvel(n2)*trc(n2)*dot_product(pframe(:,1,n2),sframe2(:,1,elside(m,ie)))+mvvel(n2)*trc(n2)*dot_product(pframe(:,2,n1),sframe2(:,1,elside(m,ie))))
            !swild10(m,2) = 0.5*(muvel(n1)*trc(n1)*dot_product(pframe(:,1,n1),sframe2(:,2,elside(m,ie)))+mvvel(n1)*trc(n1)*dot_product(pframe(:,2,n1),sframe2(:,2,elside(m,ie)))&
            !                 &+muvel(n2)*trc(n2)*dot_product(pframe(:,1,n2),sframe2(:,2,elside(m,ie)))+mvvel(n2)*trc(n2)*dot_product(pframe(:,2,n1),sframe2(:,2,elside(m,ie))))
            !swild10(m,1) = 0.5*(muvel(n1)*dot_product(pframe(:,1,n1),sframe2(:,1,elside(m,ie)))+mvvel(n1)*dot_product(pframe(:,2,n1),sframe2(:,1,elside(m,ie)))&
            !                 &+muvel(n2)*dot_product(pframe(:,1,n2),sframe2(:,1,elside(m,ie)))+mvvel(n2)*dot_product(pframe(:,2,n2),sframe2(:,1,elside(m,ie))))
            !swild10(m,2) = 0.5*(muvel(n1)*dot_product(pframe(:,1,n1),sframe2(:,2,elside(m,ie)))+mvvel(n1)*dot_product(pframe(:,2,n1),sframe2(:,2,elside(m,ie)))&
            !                 &+muvel(n2)*dot_product(pframe(:,1,n2),sframe2(:,2,elside(m,ie)))+mvvel(n2)*dot_product(pframe(:,2,n2),sframe2(:,2,elside(m,ie))))
          ...
          do j =1,i34(ie) !side
            ...
            if(id3.eq.id2) then
               ...
               if(sumtmp>0) then
                  ...
                  tmp2=max(c0,tmp3)
                  !sumtmp = sumtmp * d_tr0(n1)
                  flux_matrix00_m(indx1,n1,1,catm) = n1
                  flux_matrix00_m(indx2,n2,2,catm) = n1
                  ! n1 center, n2 dowmstream
                  if(abs(d_tr0(n2)-d_tr0(n1))>1.e-20) then
                     !r_fac(n1) = 2*(grad_x(n1)*(xnd(n2)-xnd(n1))+grad_y(n1)*(ynd(n2)-ynd(n1)))/(d_tr0(n2)-d_tr0(n1))-1
                     r_fac(n1) = 2*(grad_x(n1)*(xel(id2,ie)-xel(id,ie))+grad_y(n1)*(yel(id2,ie)-yel(id,ie)))/(d_tr0(n2)-d_tr0(n1))-1
                  ...
                  tmp2=max(c0,tmp3)
                  !sumtmp = sumtmp * d_tr0(n2)
                  flux_matrix00_m(indx1,n1,1,catm) = n2
                  flux_matrix00_m(indx2,n2,2,catm) = n2
                  ! n2 center, n1 dowmstream
                  if(abs(d_tr0(n2)-d_tr0(n1))>1.e-20) then
                     !r_fac(n2) = 2*(grad_x(n2)*(xnd(n1)-xnd(n2))+grad_y(n2)*(ynd(n1)-ynd(n2)))/(d_tr0(n1)-d_tr0(n2))-1
                     r_fac(n2) = 2*(grad_x(n2)*(xel(id,ie)-xel(id2,ie))+grad_y(n2)*(yel(id,ie)-yel(id2,ie)))/(d_tr0(n1)-d_tr0(n2))-1
               ...
               if(sumtmp>0) then
                  ...
                  tmp2=max(c0,tmp3)
                  !sumtmp = sumtmp * d_tr0(n1)
                  flux_matrix00_m(indx1,n1,2,catm) = n1
                  flux_matrix00_m(indx2,n2,1,catm) = n1
                  ! n1 center, n2 dowmstream
                  if(abs(d_tr0(n2)-d_tr0(n1))>1.e-20) then
                     !r_fac(n1) = 2*(grad_x(n1)*(xnd(n2)-xnd(n1))+grad_y(n1)*(ynd(n2)-ynd(n1)))/(d_tr0(n2)-d_tr0(n1))-1
                     r_fac(n1) = 2*(grad_x(n1)*(xel(id2,ie)-xel(id,ie))+grad_y(n1)*(yel(id2,ie)-yel(id,ie)))/(d_tr0(n2)-d_tr0(n1))-1
                  ...
                  tmp2=max(c0,tmp3)
                  !sumtmp = sumtmp * d_tr0(n2)
                  flux_matrix00_m(indx1,n1,2,catm) = n2
                  flux_matrix00_m(indx2,n2,1,catm) = n2
                  ! n2 center, n1 dowmstream
                  if(abs(d_tr0(n1)-d_tr0(n2))>1.e-20) then
                     !r_fac(n2) = 2*(grad_x(n2)*(xnd(n1)-xnd(n2))+grad_y(n2)*(ynd(n1)-ynd(n2)))/(d_tr0(n1)-d_tr0(n2))-1
                     r_fac(n2) = 2*(grad_x(n2)*(xel(id,ie)-xel(id2,ie))+grad_y(n2)*(yel(id,ie)-yel(id2,ie)))/(d_tr0(n1)-d_tr0(n2))-1
   ...
   do i=1,nx_nh
      ...
      enddo
         ...
         trc(i) = d_tr0(i) - trl0(i) !+ d_tr(i)   
         !if(trc(i)<0) write(12,*) 'negative aicen',trc_n,d_tr0(i),trl0(i)
   enddo  
    call exchange_p2d(trc)
  end subroutine tvdu_icepack_m

  !=======================================================================
subroutine tvdu_icepack_other_m(trc,trc_o,trc_n,trc_d,catm,trc_d1,trc_d2)
        
        !use mod_mesh     
     
        implicit none
        integer (kind=int_kind), intent(in) :: trc_n,trc_d,catm
        real (kind=dbl_kind), dimension(nx), intent(inout) :: trc   ,trc_o
        real (kind=dbl_kind), dimension(nx), intent(in), optional :: trc_d1,trc_d2
        real   (kind=dbl_kind)            :: fluxchan1,fluxchan2,suma,xctr2,yctr2,vnor1,vnor2,&
                                             & tmpx2,tmpx3,tmpy2,tmpy3,utmp,vtmp,xtmp,ytmp,vnorm,&
      ...
      do i=1,nx_nh
        ...
        !Wet node
          ...
          do j=1,nne(i)
            ...
            tmpnode = flux_matrix00_m(j,i,1,catm)
            tmpx2 = trc_o(tmpnode)
            if (present(trc_d1)) then
               tmpx2 = tmpx2 * trc_d1(tmpnode)
            endif
...
!            xtmp=ycj(isd2)-yctr(ie)
!            ytmp=xctr(ie)-xcj(isd2)
            tmpnode = flux_matrix00_m(j,i,2,catm)
            tmpx2 = trc_o(tmpnode)
            if (present(trc_d1)) then
               tmpx2 = tmpx2 * trc_d1(tmpnode)
            endif
      ...
      call exchange_p2d(trc)
    end subroutine tvdu_icepack_other_m

    !=======================================================================
    subroutine tvdu_icepack_dep_m(trc,trc_o,trc_n,trc_d,catm)
...
!            ytmp=xctr(ie)-xcj(isd2)
      ...
      call exchange_p2d(trc)
    end subroutine tvdu_icepack_dep_m
...
!=======================================================================
    module subroutine tracer_advection_icepack5
      ...
      !type(t_mesh),        target,       intent(in)    :: mesh
         character(len=*), parameter :: subname = '(tracer_advection_icepack5)'
...
!=======================================================================
   module subroutine tracer_advection_icepack7
      ...
      !type(t_mesh),        target,       intent(in)    :: mesh
         character(len=*), parameter :: subname = '(tracer_advection_icepack7)'
      ...
      do i=1,nx_nh
         ...
         !totaliceh = totaliceh + aice(i)*vice(i)*lump_ice_matrix(i)
         totaliceh = totaliceh + sum(vicen(i,:))*lump_ice_matrix(i)
         momentum_u = momentum_u + dot_product(vicen(i,:),m_uvel(i,:))*lump_ice_matrix(i)
         momentum_v = momentum_v + dot_product(vicen(i,:),m_vvel(i,:))*lump_ice_matrix(i)
         do n =1,ncat
            if(aicen(i,n)>puny) then
               hilyr = vicen(i,n) / aicen(i,n) /nilyr
            else
...
!upwind2
      ...
      do i=1,nx
         if(vice(i)>puny) then
            adv_threshold0(i)= 0.5 !max(c0,min(0.01,exp(-10*aice(i))/vice(i)))
         else
            adv_threshold0(i)= 0.5
         endif
      enddo
      ...
      narrays = 1
       do nt = 1, ncat
          call tvdu_icepack_m (works(:,narrays+1),m_uvel(:,nt),m_vvel(:,nt),narrays+1,nt,adv_threshold0) 
          aicen_tmp(:,nt) = works(:,narrays+1)
          narrays = narrays + 3 + ntrcr
       enddo
      ...
      swild = works(:,1)
       ...
       do nt = 1, ncat
          ...
          enddo
          call tvdu_icepack_dep_m(works(:,narrays+2),swild1,narrays+2,narrays+1,nt)
          call tvdu_icepack_dep_m(works(:,narrays+3),swild2,narrays+3,narrays+1,nt) 
          !call tvd_icepack (works(:,narrays+2),narrays+2,adv_threshold0 ) 
          !call tvd_icepack (works(:,narrays+3),narrays+3,adv_threshold0 ) 
          narrays = narrays + 3 + ntrcr
       enddo
      ...
      call work_to_other_7  (ntrcr, narr, works(:,:)) 
      
      call re_momentum_m (ntrcr, narr, works(:,:), vicen_init)
      newwork(:) = c0
      ...
      deallocate(swild2)
      deallocate(adv_threshold0)
   end subroutine tracer_advection_icepack7
...
!=======================================================================
   ...
   subroutine work_to_other_6 (ntrcr, narr, works)
        ...
        real (kind=dbl_kind),    dimension (:), allocatable ::       &
           swild
        character(len=*), parameter :: subname = '(work_to_other_6)'
...
!=======================================================================
   ...
   subroutine work_to_other_7 (ntrcr, narr, works)
        ...
        real (kind=dbl_kind),    dimension (:), allocatable ::       &
           swild
        character(len=*), parameter :: subname = '(work_to_other_7)'
        ...
        do n=1, ncat
           ...
           do it = 1, ntrcr
              if (trcr_depend(it) == 0) then
                 call tvdu_icepack_other_m(works(:,narrays+3+it),trcrn(:,it,n),narrays+3+it,narrays+1,n)
              elseif (trcr_depend(it) == 1) then
                 call tvdu_icepack_other_m(works(:,narrays+3+it),trcrn(:,it,n),narrays+3+it,narrays+2,n)
              elseif (trcr_depend(it) == 2) then
               if (it >= nt_qsno .and. it < nt_qsno+nslyr) then
                     swild = trcrn(:,it,n) + rhos*Lfresh
                     call tvdu_icepack_other_m(works(:,narrays+3+it),swild,narrays+3+it,narrays+3,n)
               else
                     call tvdu_icepack_other_m(works(:,narrays+3+it),trcrn(:,it,n),narrays+3+it,narrays+3,n)
               endif
              elseif (trcr_depend(it) == 2+nt_alvl) then
                call tvdu_icepack_other_m(works(:,narrays+3+it),trcrn(:,it,n),narrays+3+it,narrays+1,n,trcrn(:,nt_alvl,n))!narrays+3+nt_alvl)
              elseif (trcr_depend(it) == 2+nt_apnd .and. &
                      tr_pond_topo) then
               call tvdu_icepack_other_m(works(:,narrays+3+it),trcrn(:,it,n),narrays+3+it,narrays+1,n,trcrn(:,nt_apnd,n))!narrays+3+nt_apnd)
              elseif (trcr_depend(it) == 2+nt_apnd .and. &
                      tr_pond_lvl) then
                 call tvdu_icepack_other_m(works(:,narrays+3+it),trcrn(:,it,n),narrays+3+it,narrays+1,n,trcrn(:,nt_alvl,n),trcrn(:,nt_apnd,n))
                 !,narrays+3+nt_alvl,narrays+3+nt_apnd)
              elseif (trcr_depend(it) == 2+nt_fbri) then
                  call tvdu_icepack_other_m(works(:,narrays+3+it),trcrn(:,it,n),narrays+3+it,narrays+2,n,trcrn(:,nt_fbri,n))!,narrays+3+nt_fbri)
              endif
           enddo
           narrays = narrays + ntrcr +3
   ...
   end subroutine work_to_other_7
...
!=======================================================================
   ...
   subroutine re_momentum_m (ntrcr, narr, works, vicen_ini)
      ...
      real (kind=dbl_kind) :: &
         rhos       ,tmp1, tmp2, &
         Lfresh, small, sumvice, sumvice_i, utmp, vtmp, &
         &suma, puny, sumaice, momentum_ui, momentum_vi, tmpu, tmpv
      
      real(rkind) :: swild10(3,2), swild(3), swild2(3,2),fluxchan1(nx),fluxchan2(nx),aicen_af(ncat), &
         &swild3(nx)
      character(len=*), parameter :: subname = '(re_momentum_m)'
         ...
         do n=1,ncat
            ...
            do j=1,nne(i)
               ...
               do m=1,i34(ie) !side
                  ...
                  n2=isidenode(2,elside(m,ie))
                  if(ics==1) then
                     swild10(m,1) = 0.5*(m_uvel(n1,n)+m_uvel(n2,n))
                     swild10(m,2) = 0.5*(m_vvel(n1,n)+m_vvel(n2,n))
                     swild2(m,1) = m_uvel(elnode(m,ie),n)
                     swild2(m,2) = m_vvel(elnode(m,ie),n)
                  else
                     swild10(m,1) = 0.5*(m_uvel(n1,n)*dot_product(pframe(:,1,n1),eframe(:,1,ie))+m_vvel(n1,n)*dot_product(pframe(:,2,n1),eframe(:,1,ie))&
                                 &+m_uvel(n2,n)*dot_product(pframe(:,1,n2),eframe(:,1,ie))+m_vvel(n2,n)*dot_product(pframe(:,2,n2),eframe(:,1,ie)))
                     swild10(m,2) = 0.5*(m_uvel(n1,n)*dot_product(pframe(:,1,n1),eframe(:,2,ie))+m_vvel(n1,n)*dot_product(pframe(:,2,n1),eframe(:,2,ie))&
                                 &+m_uvel(n2,n)*dot_product(pframe(:,1,n2),eframe(:,2,ie))+m_vvel(n2,n)*dot_product(pframe(:,2,n2),eframe(:,2,ie)))                   
                     swild2(m,1) = m_uvel(elnode(m,ie),n)*dot_product(pframe(:,1,elnode(m,ie)),eframe(:,1,ie))+m_vvel(elnode(m,ie),n)*dot_product(pframe(:,2,elnode(m,ie)),eframe(:,1,ie))
                     swild2(m,2) = m_uvel(elnode(m,ie),n)*dot_product(pframe(:,1,elnode(m,ie)),eframe(:,2,ie))+m_vvel(elnode(m,ie),n)*dot_product(pframe(:,2,elnode(m,ie)),eframe(:,2,ie))
                  endif                     
               enddo
               ...
               tmpnode1 = flux_matrix00(j,i,1)
               tmpnode2 = flux_matrix00(j,i,2)
               fluxchan1(i) = fluxchan1(i) + (flux_matrix1(j,i,trc_d)+flux_matrix2(j,i,trc_d))*m_uvel(tmpnode1,n)/2.d0
               fluxchan2(i) = fluxchan2(i) + (flux_matrix1(j,i,trc_d)+flux_matrix2(j,i,trc_d))*m_vvel(tmpnode1,n)/2.d0
               fluxchan1(i) = fluxchan1(i) + (flux_matrix3(j,i,trc_d)+flux_matrix4(j,i,trc_d))*m_uvel(tmpnode2,n)/2.d0
               fluxchan2(i) = fluxchan2(i) + (flux_matrix3(j,i,trc_d)+flux_matrix4(j,i,trc_d))*m_vvel(tmpnode2,n)/2.d0
            enddo

            sumvice = works(i,trc_d)
            sumaice = works(i,trc_d-1)

            sumvice_i = vicen_ini(i,n)
            momentum_ui = sumvice_i * m_uvel(i,n)
            momentum_vi = sumvice_i * m_vvel(i,n)
            if(sumvice>puny) then
               m_uvel(i,n) = (momentum_ui - fluxchan1(i)/suma)/sumvice
               m_vvel(i,n) = (momentum_vi - fluxchan2(i)/suma)/sumvice
            else
               m_uvel(i,n) = 0
               m_vvel(i,n) = 0
            endif
            if(isbnd(1,i)/=0) then !b.c. (including open)
               m_uvel(i,n)=0; m_vvel(i,n)=0
            endif
            !if(sumaice<=ice_cutoff.or.sumvice<=ice_cutoff) then !no ice
            if(sumaice<=small.or.sumvice<=small) then !no ice
                  m_uvel(i,n)=uocn(i); m_vvel(i,n)=vocn(i)
            endif
            ...
            tmpu = tmpu + sumaice * m_uvel(i,n)
            tmpv = tmpv + sumaice * m_vvel(i,n)
            trc_d = trc_d + ntrcr + 3 
         enddo
      ...
      do i = 1,np
         if(sqrt(U_ice(i)**2+v_ice(i)**2)>2) then
            ...
            do n=1,ncat
               m_uvel(i,n) = m_uvel(i,n) * 2 / sqrt(tmp1**2+tmp2**2)
               m_vvel(i,n) = m_vvel(i,n) * 2 / sqrt(tmp1**2+tmp2**2)
            enddo
         endif
      enddo
      call exchange_p2d(U_ice)
    ...
    module subroutine cut_off_icepack
      ...
      logical (kind=log_kind) :: tr_brine, tr_lvl, flag_snow, flag_cold_ice, flag_warm_ice,flag_negative_salinity, &
      ...
      do n = 1, ncat                      ! For each thickness cathegory
           do i = 1, nx                   ! For each grid point
            ...
            flag_negative_salinity = .false.
            do k = 1, nilyr
               if(trcrn(i,nt_sice+k-1,n)<0.0_dbl_kind) then
                  flag_negative_salinity = .true.
               endif
            enddo
            if(flag_negative_salinity) then
               write(12,*) 'negative Sea ice salinity',trcrn(i,nt_sice:nt_sice+nilyr-1,n)
               do k = 1, nilyr
                ...
                if (flag_warm_ice) then         ! This sea ice should have melted already 
                    ...
                    if (vsnon(i,n) > small) then
                        ...
                        do k = 1, nslyr  
                           ...
                           Tmax = trcrn(i,nt_qsno+k-1,n)*puny*rnslyr / (rhos*cp_ice*vsnon(i,n))
                           !if (zTsn(k) > Tmax) flag_snow = .true.
                        end do 
                        !flag_snow = .false.

                        if (flag_snow) then
                           !write(12,*) 'flag_snow',zTsn(:)
                            trcrn(i,nt_Tsfc,n) = Tsfc
                            do k = 1, nslyr
                                 trcrn(i,nt_qsno+k-1,n) = qsn(k)
                            enddo        ! nslyr
