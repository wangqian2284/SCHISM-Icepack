...
#endif
#ifdef USE_MICE
     real(rkind), dimension(npa,30), intent(out) :: &
     &                       wave_spectrum
#endif
...
! retrieve the surface precipitation flux
        ...
        call get_wave_spec (time, wave_spectrum)
#else
...
!        real(rkind) :: utc_start   = -9999.0
        ...
        character (len=50)  :: waves_name = 'efreq'
...
! not the first call
      ...
      !-----------------------------------------------------------------------
      subroutine get_wave_spec (time, wave_spec)
        use schism_glbl, only : rkind, npa,fdb,lfdb
        use schism_msgp, only : myrank,parallel_abort
        use netcdf_io
        implicit none
        ...
        real(rkind), intent(in) :: time
        real(rkind), dimension(npa,30), intent(out) :: wave_spec
...
#ifdef DEBUG
        write(38,*)
        write(38,*) 'get_wave_spec:  time = ', time
        write(38,*) 'first_call             = ', first_call
        write(38,*) 'num_nodes_out          = ', num_nodes_out
#endif
...
! setup datasets
          dataset_1%name             = 'sflux_efreq_1'
          dataset_1%max_window_hours = 24.
          dataset_1%fail_if_missing  = .true.
          dataset_1%relative_weight  = 1.

          dataset_2%name             = 'sflux_efreq_2'
          dataset_2%max_window_hours = 24
          dataset_2%fail_if_missing  = .false.
          dataset_2%relative_weight  = 99.

! output some details
          if(myrank==0) then
            write(16,*)
            write(16,*) 'get_wave_spec:  sflux_inputs'
            write(16,*) '  start_year             = ', start_year
            write(16,*) '  start_month            = ', start_month
            write(16,*) '  start_day              = ', start_day
            write(16,*) '  start_hour             = ', start_hour
            ...
            write(16,*) '  prc_1_file             = ', &
     &                   'sflux_efreq_1'
            write(16,*) '  wave_1_max_window_hours = ', &
     &                   24.
            write(16,*) '  wave_1_fail_if_missing  = ', &
     &                   .true.
            write(16,*) '  wave_1_relative_weight  = ', &
     &                   1.
            write(16,*) '  wave_2_file             = ', &
     &                   'sflux_efreq_2'
            write(16,*) '  wave_2_max_window_hours = ', &
     &                   24
            write(16,*) '  wave_2_fail_if_missing  = ', &
     &                   .false.
            write(16,*) '  wave_2_relative_weight  = ', &
     &                   99.
            write(16,*) '  waves_name              = ', &
     &                   trim(waves_name)
          endif
...
! get the data at this time
        data_name = trim(waves_name)
        call combine_sflux_data_wave &
     &    (time_now, dataset_1, dataset_2, &
     &     dataset_1%exist, dataset_2%exist, &
     &     data_name, wave_spec, &
     &     num_nodes_out)
...
!-----------------------------------------------------------------------
      subroutine read_data_wave (file_name, data_name, data, &
     &                          nx, ny, time_num, nii)
...
!        include 'netcdf.inc'
        ...
        character(*), intent(in) ::  file_name, data_name
        integer, intent(in) :: nx, ny, time_num, nii
        real(rkind), intent(out), dimension(nx,ny) :: data
...
!-----------------------------------------------------------------------
      subroutine get_sflux_data_wave (time_now, info, data_name, &
     &                           data_out, got_suitable_bracket, &
     &                           num_nodes_out)

        use schism_glbl, only : rkind,errmsg
        use schism_msgp, only : myrank,parallel_abort,comm,rtype,nproc_schism
#ifdef USE_MICE
        use mice_module, only : ice_tr,floenum
#endif
        use netcdf_io
        implicit none
        include 'mpif.h'    
        ...
        character(*), intent(in) :: data_name
        integer, intent(in) :: num_nodes_out
        real(rkind), intent(out), dimension(num_nodes_out,30) :: &
     &    data_out
        logical, intent(out) :: got_suitable_bracket
        ...
        real(rkind), dimension(info%nx,info%ny) :: data_tmp
        real(rkind) window_hours
        integer i,time_num_1, time_num_2
        integer j,k,i1,j1,maxloc1,i_elem,istat
        real(rkind), dimension(info%nx,info%ny) :: hice1,floenum1,data_tmp1,data_tmp2,data_tmp3, &
        & data_tmp01,data_tmp02,data_tmp03
        real(rkind), dimension(30) :: wavefreq, waveT
        real(rkind) alpha,hice
        logical findice
        logical got_bracket
        character(len=100) :: msg_tmp
...
! attenuated by ice, follow Roach et al., 2018
! only draw in ocean swell along lines of constant longitude

        do k = 1, 30
                wavefreq(k)=0.0345227*(1.1**(k-1))
                waveT(k) = 1/wavefreq(k)
        enddo
        ...
        do i=1,num_nodes_out
                i_elem = info%in_elem_for_out_node(i)
                hice = 0
                if(ice_tr(2,i)>0.01) hice=ice_tr(1,i)/ice_tr(2,i)
                do j =1,3
                        i1 = info%node_i(info%elem_nodes(i_elem,j))
                        j1 = info%node_j(info%elem_nodes(i_elem,j))
                        data_tmp1(i1,j1) = data_tmp1(i1,j1) + hice * info%weight(i,j)
                        data_tmp2(i1,j1) = data_tmp2(i1,j1) + floenum(i) * info%weight(i,j)
                        data_tmp3(i1,j1) = data_tmp3(i1,j1) + info%weight(i,j)
                enddo
        enddo


        call mpi_allreduce(data_tmp1,data_tmp01,info%nx*info%ny,rtype,MPI_SUM,comm,istat)
        call mpi_allreduce(data_tmp2,data_tmp02,info%nx*info%ny,rtype,MPI_SUM,comm,istat)
        call mpi_allreduce(data_tmp3,data_tmp03,info%nx*info%ny,rtype,MPI_SUM,comm,istat)
        hice1 = data_tmp01 / data_tmp03
        floenum1 = data_tmp02 / data_tmp03
        do j = 1,info%nx
                do k = 1,info%ny
                        if(data_tmp03(j,k)==0) then
                                hice1(j,k) = 0
                                floenum1(j,k) = 0
                        endif
                enddo
        ...
        if (got_suitable_bracket) then
          ...
          time_num_2 = time_num_1 + 1
                do i=1,30
                        ...
                        &       info%file_num_for_time(time_num_2), &
                        &       info%time_num_for_time(time_num_2), &
                        &       data_tmp, info%nx, info%ny, i)
                        do j = 1,info%nx
                                findice=.false.
                                do k = 1,info%ny
                                        if(hice1(j,k)>0 .and. (findice==.false.)) then
                                                findice=.true.
                                        else
                                                data_tmp1(j,k) = data_tmp(j,k)
                                                data_tmp01(j,k) = data_tmp(j,k)
                                        endif
                                        if(data_tmp03(j,k)==0) then
                                                data_tmp1(j,k) = 0
                                                data_tmp01(j,k) = 0
                                        ...
                                        if(findice) then
                                                alpha = exp(-0.3203+2.058*hice1(j,k-1)-0.9375*waveT(i)-0.4269*hice1(j,k-1)**2+0.1566*hice1(j,k-1)*waveT(i)+0.0006*waveT(i)**2)
                                                data_tmp1(j,k) = min(data_tmp(j,k),data_tmp1(j,k-1)*exp(-1*alpha*floenum1(j,k-1)*6356*1000*0.5/180*3.1415926))
                                                data_tmp01(j,k) = 0
                                        endif
                                        if((hice1(j,k)==0) .and. (data_tmp03(j,k)>0) ) then
                                                findice=.false.
                                                data_tmp1(j,k) = data_tmp(j,k)
                                                data_tmp01(j,k) = data_tmp(j,k)
                                        endif
                        ...
                        do k = 1,info%ny
                                ...
                                do j = 2,info%nx
                                        if(hice1(j,k)>0 .and. (findice==.false.)) then
                                                ...
                                                data_tmp2(j,k) = data_tmp(j,k)
                                        endif
                                        if(data_tmp03(j,k)==0) data_tmp2(j,k) = 0
                                        if(findice) then
                                                alpha = exp(-0.3203+2.058*hice1(j-1,k)-0.9375*waveT(i)-0.4269*hice1(j-1,k)**2+0.1566*hice1(j-1,k)*waveT(i)+0.0006*waveT(i)**2)
                                                data_tmp2(j,k) = min(data_tmp(j,k),data_tmp2(j-1,k)*exp(-1*alpha*floenum1(j-1,k)*6378*cos(info%lat(j-1,k))*1000*0.5/180*3.1415926))
                                        endif
                                        if((hice1(j,k)==0) .and. (data_tmp03(j,k)>0) ) then
                                                findice=.false.
                                                data_tmp2(j,k) = data_tmp(j,k)
                                ...
                                enddo
                                data_tmp2(1,k) = data_tmp2(info%nx,k) 
                        enddo
                        ...
                        do k = 1,info%ny
                                ...
                                do j = info%nx-1,1,-1
                                        if(hice1(j,k)>0 .and. (findice==.false.)) then
                                                ...
                                                data_tmp3(j,k) = data_tmp(j,k)
                                        endif
                                        if(data_tmp03(j,k)==0) data_tmp3(j,k) = 0
                                        if(findice) then
                                                alpha = exp(-0.3203+2.058*hice1(j+1,k)-0.9375*waveT(i)-0.4269*hice1(j+1,k)**2+0.1566*hice1(j+1,k)*waveT(i)+0.0006*waveT(i)**2)
                                                data_tmp3(j,k) = min(data_tmp(j,k),data_tmp3(j+1,k)*exp(-1*alpha*floenum1(j+1,k)*6378*cos(info%lat(j-1,k))*1000*0.5/180*3.1415926))
                                        endif
                                        if((hice1(j,k)==0) .and. (data_tmp03(j,k)>0) ) then
                                                findice=.false.
                                                data_tmp3(j,k) = data_tmp(j,k)
                                ...
                                enddo
                                data_tmp3(info%nx,k) = data_tmp3(1,k) 
                        enddo
                        
                        data_tmp = data_tmp1+data_tmp2+data_tmp3-data_tmp01-data_tmp01

                        ! now the data needs to be interpolated in space from the original data
                        ! grid to the model grid
                                call interp_grid (data_out(:,i), data_tmp, info%weight, &
                        &                      info%in_elem_for_out_node, &
                        &                      info%elem_nodes, info%num_elems, &
                        &                      info%node_i, info%node_j, &
                        &                      info%num_nodes, num_nodes_out, &
...
!-----------------------------------------------------------------------
      subroutine interp_time_wave (dataset_name, time, data_name, &
     &                        time_1, file_num_1, file_time_1, &
     &                        time_2, file_num_2, file_time_2, &
     &                        data_out, nx, ny, nii)
     ...
     &                         nx, ny, nii
        ...
        call read_data_wave (file_name_1, data_name, data_1, &
     &                  nx, ny, file_time_1, nii)

        call read_data_wave (file_name_2, data_name, data_2, &
     &                  nx, ny, file_time_2, nii)
...
!-----------------------------------------------------------------------
      subroutine combine_sflux_data_wave (time_now, info_1, info_2, &
     &                               read_1, read_2, &
     &                               data_name, data_out, &
     &                               num_nodes_out)
     ...
     &    data_out
        ...
        real(rkind), dimension(num_nodes_out,30) :: data_1, data_2
        real(rkind) local_weight_2, sum_weights
        logical got_data_1, got_data_2, bad_node_2
        integer i_node
...
! attempt to read data_1 and data_2 (only if input flags specify)
        if (read_1) then
          call get_sflux_data_wave (time_now, info_1, data_name, &
     &                         data_1, got_data_1, num_nodes_out)
        endif

        if (read_2) then
          call get_sflux_data_wave (time_now, info_2, data_name, &
     &                         data_2, got_data_2, num_nodes_out)
        endif
