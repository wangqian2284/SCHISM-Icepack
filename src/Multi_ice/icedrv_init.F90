...
!=======================================================================
      ...
      subroutine init_coupler_flux()
          ...
          strocnxTn(:,:)= c0   ! ice-ocean stress for each category, x-direction
          strocnyTn(:,:)= c0   ! ice-ocean stress for each category, y-direction
          fresh    (:) = c0
          fresh_tot(:) = c0
          fsalt    (:) = c0
          fhocn    (:) = c0
...
!=======================================================================
      ...
      subroutine init_state_var ()
         ...
         !   ainit = c0
         !   hinit = c0
         !   ainit(2) = 0.4  ! assumes we are using the default ITD boundaries
         !   hinit(2) = 1.d0
         !   ainit(5) = 0.4  ! assumes we are using the default ITD boundaries
         !   hinit(5) = 5.d0
          do i = 1, nx
            do n = 1, ncat
...
!=======================================================================
      ...
      subroutine mice_hotstart_var ()
        ...
        call icepack_warnings_flush(nu_diag)
         ...
         allocate(swild4(np_global,ncat+1),swild3(ncat+1,np_global),swild2(ncat,np_global),swild(np_global,ncat)) 
        ...
        if(j/=NF90_NOERR) call parallel_abort('mice_init: hotstart.nc not found')
         ...
         if(myrank==0) then
            ...
            j=nf90_get_var(ncid2,mm,swild2(:,:),(/1,1/),(/ncat,np_global/))
            if(j/=NF90_NOERR) call parallel_abort('mice_init: nc aicen2')
         endif
         call mpi_bcast(swild2,ncat*np_global,rtype,0,comm,istat)
         swild = transpose(swild2)
        do i=1,np_global
          if(ipgl(i)%rank==myrank) then
            ip=ipgl(i)%id
            aicen(ip,1:ncat) = swild(i,:)
        ...
        enddo
         ...
         do i=1,np_global
            if(ipgl(i)%rank==myrank) then
               ip=ipgl(i)%id
               trcrn(ip,nt_Tsfc,1:ncat) = swild(i,:)
            endif
         enddo

          if(myrank==0) then
             j=nf90_inq_varid(ncid2, "sstn_old",mm)
             if(j/=NF90_NOERR) call parallel_abort('mice_init: nc sstn_old1')
             j=nf90_get_var(ncid2,mm,swild3(:,:),(/1,1/),(/ncat+1,np_global/))
             if(j/=NF90_NOERR) call parallel_abort('mice_init: nc sstn_old2')
          endif
          call mpi_bcast(swild3,(ncat+1)*np_global,rtype,0,comm,istat)
          swild4 = transpose(swild3)
          do i=1,np_global
             if(ipgl(i)%rank==myrank) then
                ip=ipgl(i)%id
                !j=nf90_get_var(ncid2,mm,tr_nd0(:,:,ip),(/1,1,i/),(/ntracers,nvrt,1/))
                j=nf90_get_var(ncid2,mm,swild3(:,ip),(/1,i/),(/ncat+1,1/))
                sstn_old(ip,1:ncat+1) = swild4(i,:)
             endif
          enddo

          if(myrank==0) then
             j=nf90_inq_varid(ncid2, "hocnn_old",mm)
             if(j/=NF90_NOERR) call parallel_abort('mice_init: nc hocnn_old1')
             j=nf90_get_var(ncid2,mm,swild3(:,:),(/1,1/),(/ncat+1,np_global/))
             if(j/=NF90_NOERR) call parallel_abort('mice_init: nc hocnn_old2')
          endif
          call mpi_bcast(swild3,(ncat+1)*np_global,rtype,0,comm,istat)
          swild4 = transpose(swild3)
          do i=1,np_global
             if(ipgl(i)%rank==myrank) then
                ...
                !j=nf90_get_var(ncid2,mm,tr_nd0(:,:,ip),(/1,1,i/),(/ntracers,nvrt,1/))
                j=nf90_get_var(ncid2,mm,swild3(:,ip),(/1,i/),(/ncat+1,1/))
                hocnn_old(ip,1:ncat+1) = swild4(i,:)
             endif
          enddo
         
      
          if(myrank==0) then
             j=nf90_inq_varid(ncid2, "fswthrun_old",mm)
             if(j/=NF90_NOERR) call parallel_abort('mice_init: nc fswthrun_old1')
             j=nf90_get_var(ncid2,mm,swild3(:,:),(/1,1/),(/ncat+1,np_global/))
             if(j/=NF90_NOERR) call parallel_abort('mice_init: nc fswthrun_old2')
          endif
          call mpi_bcast(swild3,(ncat+1)*np_global,rtype,0,comm,istat)
          swild4 = transpose(swild3)
          do i=1,np_global
             if(ipgl(i)%rank==myrank) then
                ...
                !j=nf90_get_var(ncid2,mm,tr_nd0(:,:,ip),(/1,1,i/),(/ntracers,nvrt,1/))
                j=nf90_get_var(ncid2,mm,swild3(:,ip),(/1,i/),(/ncat+1,1/))
                fswthrun_old(ip,1:ncat+1) = swild4(i,:)
             endif
          enddo
        ...
        if (tr_iage) then
         ...
         do i=1,np_global
            if(ipgl(i)%rank==myrank) then
               ip=ipgl(i)%id
               trcrn(ip,nt_iage,1:ncat) = swild(i,:)
            endif
         enddo
      ...
      if (tr_lvl) then
         ...
         do i=1,np_global
            if(ipgl(i)%rank==myrank) then
               ip=ipgl(i)%id
               trcrn(ip,nt_alvl,1:ncat) = swild(i,:)
            endif
         enddo
         ...
         do i=1,np_global
            if(ipgl(i)%rank==myrank) then
               ip=ipgl(i)%id
               trcrn(ip,nt_vlvl,1:ncat) = swild(i,:)
            endif
         enddo
      ...
      if (tr_pond_topo) then
         ...
         do i=1,np_global
            if(ipgl(i)%rank==myrank) then
               ip=ipgl(i)%id
               trcrn(ip,nt_apnd,1:ncat) = swild(i,:)
            endif
         enddo
         ...
         do i=1,np_global
            if(ipgl(i)%rank==myrank) then
               ip=ipgl(i)%id
               trcrn(ip,nt_hpnd,1:ncat) = swild(i,:)
            endif
         enddo
         ...
         do i=1,np_global
            if(ipgl(i)%rank==myrank) then
               ip=ipgl(i)%id
               trcrn(ip,nt_ipnd,1:ncat) = swild(i,:)
            endif
         enddo
      ...
      if (tr_brine) then
         ...
         do i=1,np_global
            if(ipgl(i)%rank==myrank) then
               ip=ipgl(i)%id
               trcrn(ip,nt_fbri,1:ncat) = swild(i,:)
            endif
         enddo
         ...
         do i=1,np_global
            if(ipgl(i)%rank==myrank) then
               ip=ipgl(i)%id
               first_ice_real(ip,1:ncat) = swild(i,:)
            endif
         enddo
      ...
      do k = 1,nilyr
        ...
        units='psu'
         ...
         do i=1,np_global
            if(ipgl(i)%rank==myrank) then
               ip=ipgl(i)%id
               trcrn(ip,nt_sice+k-1,1:ncat) = swild(i,:)
            endif
         enddo
      ...
      do k = 1,nilyr
        ...
        units='J/m3'
         if(myrank==0) then
            j=nf90_inq_varid(ncid2, trim(trname),mm)
            if(j/=NF90_NOERR) call parallel_abort('mice_init: nc qicen_a')
            j=nf90_get_var(ncid2,mm,swild2(:,:),(/1,1/),(/ncat,np_global/))
            if(j/=NF90_NOERR) call parallel_abort('mice_init: nc qicen_b')
         endif
         call mpi_bcast(swild2,ncat*np_global,rtype,0,comm,istat)
         swild = transpose(swild2)
         do i=1,np_global
            if(ipgl(i)%rank==myrank) then
               ip=ipgl(i)%id
               trcrn(ip,nt_qice+k-1,1:ncat) = swild(i,:)
            endif
         enddo
     ...
     do k = 1,nslyr
        ...
        units='J/m3'
         if(myrank==0) then
            j=nf90_inq_varid(ncid2, trim(trname),mm)
            if(j/=NF90_NOERR) call parallel_abort('mice_init: nc qsnon_a')
            j=nf90_get_var(ncid2,mm,swild2(:,:),(/1,1/),(/ncat,np_global/))
            if(j/=NF90_NOERR) call parallel_abort('mice_init: nc qsnon_b')
         endif
         call mpi_bcast(swild2,ncat*np_global,rtype,0,comm,istat)
         swild = transpose(swild2)
         do i=1,np_global
            if(ipgl(i)%rank==myrank) then
               ip=ipgl(i)%id
               trcrn(ip,nt_qsno+k-1,1:ncat) = swild(i,:)
            endif
         enddo
      ...
      if (tr_snow) then
         do k=1,nslyr
            ...
            do i=1,np_global
               if(ipgl(i)%rank==myrank) then
                  ip=ipgl(i)%id
                  trcrn(ip,nt_smice+k-1,1:ncat) = swild(i,:)
               endif
            enddo
         enddo
         do k=1,nslyr
            ...
            do i=1,np_global
               if(ipgl(i)%rank==myrank) then
                  ip=ipgl(i)%id
                  trcrn(ip,nt_smliq+k-1,1:ncat) = swild(i,:)
               endif
            enddo                    
         enddo
         do k=1,nslyr
            ...
            do i=1,np_global
               if(ipgl(i)%rank==myrank) then
                  ip=ipgl(i)%id
                  trcrn(ip,nt_rhos+k-1,1:ncat) = swild(i,:)
               endif
            enddo           
         enddo
         do k=1,nslyr
            ...
            do i=1,np_global
               if(ipgl(i)%rank==myrank) then
                  ip=ipgl(i)%id
                  trcrn(ip,nt_rsnw+k-1,1:ncat) = swild(i,:)
               endif
            enddo         
         enddo         
      ...
      if (tr_fsd) then
         do k=1,nfsd
            write(trname,'(A4,I2.2)') 'fsd_', k
            write(longname,'(A23,I2.2)') 'floe size distribution:', k
            if(myrank==0) then
               j=nf90_inq_varid(ncid2, trim(trname),mm)
            ...
            do i=1,np_global
               if(ipgl(i)%rank==myrank) then
                  ip=ipgl(i)%id
                  trcrn(ip,nt_fsd+k-1,1:ncat) = swild(i,:)
               endif
            enddo    
         enddo
         !trcrn(:,nt_fsd+nfsd-1,1:ncat) = 1.d0
      endif
        j=nf90_close(ncid2)
        if(j/=NF90_NOERR) call parallel_abort('mice_init: nc close')
      deallocate(swild,swild2,swild3,swild4)
...
!=======================================================================
      ...
      subroutine mice_hotstart_HYCOM_var ()
         use schism_glbl, only:in_dir,np,len_in_dir,np_global,ipgl,npa
         use schism_msgp, only: myrank,rtype,comm,parallel_abort,exchange_p2d
         use icepack_intfc, only: icepack_aggregate,icepack_init_fsd
        ...
        logical (kind=log_kind)   ::                        &
             solve_zsal, skl_bgc, z_tracers,                &
             tr_iage, tr_FY, tr_lvl, tr_aero, tr_fsd,       &
             tr_pond_topo, tr_pond_lvl, tr_brine,           &
             tr_bgc_N, tr_bgc_C, tr_bgc_Nit,                &
             tr_bgc_Sil,  tr_bgc_DMS,                       &
             tr_bgc_chl,  tr_bgc_Am,                        &
        ...
        call icepack_query_tracer_indices(                                          &
             ...
             nt_iage_out=nt_iage, nt_FY_out=nt_FY,                                  &
             nt_qice_out=nt_qice, nt_sice_out=nt_sice, nt_fbri_out=nt_fbri,         &
             nt_aero_out=nt_aero, nt_qsno_out=nt_qsno, nt_fsd_out=nt_fsd)
        call icepack_query_parameters(solve_zsal_out=solve_zsal,                    &
             skl_bgc_out=skl_bgc, z_tracers_out=z_tracers, ktherm_out=ktherm)
        call icepack_query_tracer_flags(                                            &
             tr_iage_out=tr_iage, tr_FY_out=tr_FY, tr_lvl_out=tr_lvl,               &
             tr_aero_out=tr_aero, tr_fsd_out=tr_fsd,                                &
             tr_pond_topo_out=tr_pond_topo, tr_pond_lvl_out=tr_pond_lvl,            &
             tr_brine_out=tr_brine, tr_bgc_N_out=tr_bgc_N, tr_bgc_C_out=tr_bgc_C,   &
             tr_bgc_Nit_out=tr_bgc_Nit, tr_bgc_Sil_out=tr_bgc_Sil,                  &
             tr_bgc_DMS_out=tr_bgc_DMS,                                             &
                  ...
                  !write(*,*) i,lat_val(i)*180/pi,lat_val(i)
                    ...
                    vsnon(i,n) = c0
                     ...
                     if (tr_fsd) call icepack_init_fsd(nfsd=nfsd, ice_ic=ice_ic, &
                                       ...
                                       floe_binwidth=floe_binwidth,       &
                                       afsd=trcrn(i,nt_fsd:nt_fsd+nfsd-1,n))    
                     trcrn(i,nt_fsd:nt_fsd+nfsd-1,n) = 0.d0
                     trcrn(i,nt_fsd+nfsd-1,n) = 1.0    
               endif
            enddo                  ! ncat
                call icepack_warnings_flush(ice_stderr)
                if (icepack_warnings_aborted()) call icedrv_system_abort(string=subname, &
