...
!            araftn(n) = mraftn(n)*ardg1n(n)*p5
      ...
      if (present(fhocn)) then
         ...
         do n=1,ncat
            hocnn(n+1)=hocnn(n+1)+esnow_mltn(n)*dti
         enddo
      endif
...
! ECH note:  the following arrays only need be defined on iridge cells
      ...
      !-----------------------------------------------------------------
            ...
            do k = 1, nslyr
               ...
               esnow_mltn(n)=esnow_mltn(n)+esrdgn*(c1-fsnowrdg)
            enddo
      ...
      !-----------------------------------------------------------------
            ...
            do it = 1, ntrcr
               ...
               !if(abs(work)>abs(atrcrn(it,n))) write(12,*) ' Ridging error:', work,atrcrn(it,n),trfactor(:)
               !atrcrn(it,n) = atrcrn(it,n) * (c1-min(c1,abs(work/(atrcrn(it,n)+puny))))
      ...
      !-----------------------------------------------------------------
               ...
               !if(farea*ardg2n<c0) write(12,*) ' Ridging error:2', aicen(nr),farea,ardg2n
               !if(fvol *virdgn<c0) write(12,*) ' Ridging error:3', vicen(nr),fvol,virdgn
               !if(fvol *vsrdgn*fsnowrdg<c0) write(12,*) ' Ridging error:4', vsnon(nr),fvol,vsrdgn,fsnowrdg
