...
subroutine step_therm1 (dt)
    ...
    do i = 1, nx
      ...
      call icepack_step_therm1(dt=dt, ncat=ncat, nilyr=nilyr, nslyr=nslyr, &
          ...
          strocnxTn= strocnxTn(i,:),strocnyTn = strocnyTn(i,:), &
          floediamsstn = diametersstn,                          &
          fbot     = fbot(i),       frzmlt    = frzmlt(i),      &
          fbotn    = fbotn(i,:),    frzmltn   = frzmltn(i,:),   &
...
subroutine ocean_mixed_layer (dt)
    ...
    do i = 1, nx
       call ocn_mixed_layer_icepack( alvdr_ocn=alvdr_ocn(i),  swvdr=swvdr(i),         & 
                                     ...
                                     flwout_ocn=flwout_ocn(i),sst=sstn(i,1),             &
                                     ...
                                     fswthrun_old=fswthrun_old(i,1), hocnn=hocnn(i,1),&
                                     frzmltn=frzmltn(i,:),                            &
                                     sss=sss(i),              dt =dt         ) 
...
subroutine ocn_mixed_layer_icepack(                       &
    ...
    real (kind=dbl_kind), dimension (:), intent(inout) :: &
       frzmltn   ! freezing/melting potential (W/m^2) for each category
   ...
   fswthrun_old = swabs
   hocnn = hocnn + fsens_ocn + flat_ocn + flwout_ocn + flw- Lfresh*fsnow
...
module subroutine step_icepack()
   ...
   &lice_free_gb,lhas_ice,errmsg,ice_evap,isbnd,nnp,indnd,nstep_ice,it_main,ipgl
    use schism_msgp, only : myrank,nproc,parallel_abort,comm,ierr,exchange_p2d,rtype
    use mice_module
    use mice_therm_mod  
    ...
    integer (kind=int_kind) :: &
       i,j,k,jj,njj,kk, nt_Tsfc,nt_sice               ! dynamics supercycling index
    ...
    real (kind=dbl_kind) :: &
       ...
       t1, t2, t3, t4,dt,umod,tmp1,tmp2,tmp3,&
       dux,dvy,aux,puny,hice,totalicea,total_a,totaliceh,total_h

    real(kind=dbl_kind) :: &
       swild(npa)
    real (kind=dbl_kind), dimension(npa,ncat+1) :: &
       aice_isst             ! initial ice concentration for sstn
    logical :: lice_free
    ...
    do i=1,npa
      aice_isst(i,1) = aice0(i)
      tmp1 = aice0(i)*sstn(i,1)
      tmp2 = aice0(i)*sstn_old(i,1)
      do j=1,ncat
         aice_isst(i,j+1) = aicen(i,j)
         tmp1 = tmp1 + aicen(i,j)*sstn(i,j+1)
         tmp2 = tmp2 + aicen(i,j)*sstn_old(i,j+1)
      enddo
      !if(abs(tmp1-sst(i))>0.0001.and.lhas_ice(i)) write(12,*) 'puzzle sstn',aice0(i),aicen(i,:),sstn(i,:),sst(i),hocnn_old(i,:),netheatiold(i)
      !if(abs(tmp1-sst(i))>1 .and. lhas_ice(i)) write(12,*) 'puzzle sstn',aice0(i),aicen(i,:),sstn(i,:),sstn_old(i,:),sst(i),tmp1,sstiold(i),tmp2
      !if(aice0(i)<1) write(12,*) 'puzzle sstn',aice0(i),aicen(i,:),sstn(i,:),sstn_old(i,:),sst(i),tmp1,sstiold(i),tmp2
    enddo
    !-----------------------------------------------------------------
    ...
    do k = 1, ndtd !split in time; =1 in icedrv_set.F90
       ...
       if(ievp==1) then
         ...
         call ice_mmevp !this subroutine has multi-class ice velocitis 
         if(myrank==0) write(16,*)'done ice mmEVP dynamics'
       else
       ...
       else if (ice_advection==8) then
         call tracer_advection_icepack7
         if(myrank==0) write(16,*)'done ice TVD-UPWIND advection for mmevp'
       else
...
srad_th_ice(:)=0
 ...
 do i=1,npa
   !if(lhas_ice(i)) then
     ...
     !srad_th_ice(i)     =  fswthrun_old(i,1)*aice0(i)
     net_heat_flux(i)   =  hocnn(i,1)*aice0(i)
     do kk=1,ncat
      srad_th_ice(i)   = srad_th_ice(i) + fswthrun(i,kk)*aicen(i,kk)
      net_heat_flux(i) = net_heat_flux(i) + hocnn(i,kk+1)*aicen(i,kk)
     enddo
   !endif

!redistribution sstn
      tmp1 = max(aice_isst(i,1) - aice0(i), 0.d0)*sstn(i,1)
      tmp2 = max(aice0(i) - aice_isst(i,1), 0.d0)
      do j=1,ncat
         tmp1 = tmp1 + max(aice_isst(i,j+1) - aicen(i,j), 0.d0)*sstn(i,j+1)
         tmp2 = tmp2 + max(aicen(i,j) - aice_isst(i,j+1), 0.d0)
      enddo

      if(tmp2>puny) then
         if(aice0(i)>puny) then
            sstn(i,1) = ((aice_isst(i,1)*sstn(i,1))+ tmp1 / tmp2 * max(aice0(i) - aice_isst(i,1) , 0.d0)) / (max(aice0(i) , aice_isst(i,1)))
            !sstn(i,1) = max(min(sstn(i,1),sst(i)+1),sst(i)-1)
         else
            sstn(i,1) = sst(i)
         endif
         ...
         do j=1,ncat
            if(aicen(i,j)>puny) then
               sstn(i,j+1) = ((aice_isst(i,j+1)*sstn(i,j+1))+ tmp1 / tmp2 * max(aicen(i,j) - aice_isst(i,j+1), 0.d0)) / max(aicen(i,j) , aice_isst(i,j+1))
               !sstn(i,j+1) = max(min(sstn(i,j+1),sst(i)+1),sst(i)-1)
            else
               sstn(i,j+1) = sst(i)
            endif
         enddo
      endif
      !write(12,*) 'sstn redis', tmp1,tmp2,sstn(i,:),aice_isst(i,:),aice0(i),aicen(i,:),fswthrun(i,:),hocnn(i,:)
 !Check outputs
 ...
 !&fresh_wa_flux(i),net_heat_flux(i),ice_tr(:,i),ice_evap(i)
...
sstn_old(:,:) = sstn(:,:)
fswthrun_old(:,2:ncat+1)=fswthrun(:,:)
hocnn_old(:,:)=hocnn(:,:)
