...
!         Elizabeth C. Hunke, LANL
      ...
      real (kind=dbl_kind), dimension(:), intent(in) :: &   
         sstn          , &! SST for each ice category and open water
         frzmltn       , &! freezing/melting potential (W/m^2)for each category
         strocnxTn     , &! ice-ocean stress for each category, x-direction
         strocnyTn        ! ice-ocean stress for each category, y-direction

      real (kind=dbl_kind), dimension(:), intent(out) :: &   
         fbotn           ! ice-ocean heat flux at bottom surface (W/m^2) for each category
   ...
   ! a new scheme to get fbot for each category
   ! lateral melting is due to open water(frzmltn(1) and sstn(1))
   ! fbotn is bottom melting for each ice category(frzmltn(2:ncat+1) and sstn(2:ncat+1))
      ...
      do n = 1, ncat
         if (aicen(n) > puny .and. frzmltn(n+1) < c0) then ! ice can melt
            deltaT = max((sstn(n+1)-Tbot),c0)
            ! strocnx has units N/m^2 so strocnx/rho has units m^2/s^2
            !ustar = sqrt (sqrt(strocnxT**2+strocnyT**2)/rhow)
            ustar = sqrt (sqrt(strocnxTn(n)**2+strocnyTn(n)**2)/rhow)
            ...
            fbotn(n) = cpchr * deltaT * ustar ! < 0
            fbotn(n) = max (fbotn(n), frzmltn(n+1)) ! frzmlt < fbot < 0           
         endif
         etotn(n) = c0
            ! melting energy/unit area in each column, etot < 0

            do k = 1, nslyr
               etotn(n) = etotn(n) + qsnon(k,n) * vsnon(n)/real(nslyr,kind=dbl_kind)
            enddo

            do k = 1, nilyr
               etotn(n) = etotn(n) + qicen(k,n) * vicen(n)/real(nilyr,kind=dbl_kind)
            enddo                  ! nilyr
      enddo
      ...
      if(frzmltn(1) < c0 .and. aice>puny) then
         deltaT = max((sstn(1)-Tbot),c0)
         wlat_loc = m1 * deltaT**m2 ! Maykut & Perovich
         if(floediamsstn>c1) then
            rside = wlat_loc*dt*pi/(floeshape*floediamsstn) ! sstn test
         else
...
!          Elizabeth C. Hunke, LANL
      ...
      real (kind=dbl_kind), intent(inout) :: &
         ...
         strocnxT    , & ! ice-ocean stress, x-direction
         strocnyT    , & ! ice-ocean stress, y-direction
         floediamsstn, & ! mean diameter of floe size
         fbot        , & ! ice-ocean heat flux at bottom surface (W/m^2)
         frzmlt      , & ! freezing/melting potential         (W/m^2)
         rside       , & ! fraction of ice that melts laterally
         fside       , & ! lateral heat flux                  (W/m^2)
