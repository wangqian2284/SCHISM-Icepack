! Modified EVP dynamics of multi-ice module (Bouillon 2013)
subroutine ice_mmevp
   ...
   &elside,mnei,rho0,idry,errmsg,pframe,eframe,indnd,nnp,omega_e,xlon,ylat,idry_e,pr
    ...
    implicit none
    include 'mpif.h'
    integer :: isub,i,j,ie,n1,n2,icount,m,id,n
   ...
   &zeta,delta_nd,sum1,sum2,dt_by_mass,tmp3,tmp4
    ...
    integer :: iball(mnei)
    real(rkind) :: swild(2,3),deta(2,nea),swild3(npa), &
   ...
   &deta_pice(2,nea),p_ice(3),mU_ice_0(npa,ncat),mV_ice_0(npa,ncat),m_uvel0(npa,ncat),m_vvel0(npa,ncat)
    logical :: &
      calc_strair
    ...
    mU_ice_0=m_uvel0;mV_ice_0=m_vvel0
  ...
  !    endif
      ...
      enddo !i
        ...
        do i=1,np !resident
          ...
          tmp2 = (dt_by_mass/area_median(i)*sum1)**2 + (dt_by_mass/area_median(i)*sum2)**2 !ice stress

          tmp3 = 0.d0 !compare other force with ice stress
          if(abs(tmp2)>0) tmp3 = tmp1/tmp2

          !if(a_ice0(i)>0.85) then
          !if(tmp3<1.d0) then
          if(sstnbeta(i)<0.1) then
            ...
            do n=1,ncat
              if(isbnd(1,i)/=0) then !b.c. (including open)
                m_uvel0(i,n)=0; m_vvel0(i,n)=0
                cycle
              endif
        
              iball(1:nne(i))=indel(1:nne(i),i)
              if(a_icen0(i,n)>ice_cutoff) then
                tmp3= v_icen0(i,n) / a_icen0(i,n) !thickness of ice
                tmp4= ice_tr0(3,i) / ice_tr0(2,i) !thickness of snow, assume snow cover is uniform
              endif
              if(a_icen0(i,n)<=ice_cutoff.or.v_icen0(i,n)<=ice_cutoff) then !no ice
                  !U_ice(i)=0; V_ice(i)=0
                  m_uvel0(i,n)=u_ocean(i); m_vvel0(i,n)=v_ocean(i)
                cycle 
              endif
              ...
              !Not bnd node; has ice
              mass=(rhoice*tmp3*a_icen0(i,n)+rhosno*tmp4*a_icen0(i,n))  !>0
              mass=max(mass,9.d0*a_icen0(i,n)) !limit m/a>=9
              !Coriolis @ node
              !cori_nd=dot_product(weit_elem2node(1:nne(i),i),swild2(iball(1:nne(i))))
              cori_nd = 2.d0*omega_e*sin(ylat(i))
              !Debug
              ...
              !RHS
              !Error: tri only
              umod=sqrt((m_uvel0(i,n)-u_ocean(i))**2+(m_vvel0(i,n)-v_ocean(i))**2)
              dt_by_mass=dt_dyn/mass
              gam1=a_icen0(i,n)*dt_by_mass*Cdn_ocn(i)*rhowat*umod
              !gam1=ice_tr0(2,i)*dt_by_mass*cdwat*rhowat*umod

              if (calc_strair) then
                  rx=mevp_alpha2*m_uvel0(i,n)+mU_ice_0(i,n)+gam1*(u_ocean(i)*cos_io-v_ocean(i)*sin_io)+ &
                &dt_by_mass*stress_atmice_x(i)/ice_tr0(2,i)*a_icen0(i,n)
                  ry=mevp_alpha2*m_vvel0(i,n)+mV_ice_0(i,n)+gam1*(u_ocean(i)*sin_io+v_ocean(i)*cos_io)+ &
                &dt_by_mass*stress_atmice_y(i)/ice_tr0(2,i)*a_icen0(i,n)
              else
                  rx=mevp_alpha2*m_uvel0(i,n)+mU_ice_0(i,n)+gam1*(u_ocean(i)*cos_io-v_ocean(i)*sin_io)+ &
                &dt_by_mass*a_icen0(i,n)*stress_atmice_x(i)
                  ry=mevp_alpha2*m_vvel0(i,n)+mV_ice_0(i,n)+gam1*(u_ocean(i)*sin_io+v_ocean(i)*cos_io)+ &
                &dt_by_mass*a_icen0(i,n)*stress_atmice_y(i)
              endif
              ...
              tmp1=1+mevp_alpha2+gam1*cos_io+Tbu(i)/(sqrt(mU_ice_0(i,n)**2+mV_ice_0(i,n)**2)+5e-5)
              tmp2=dt_dyn*cori_nd+gam1*sin_io
              delta=tmp1*tmp1+tmp2*tmp2
              if(delta<=0) call parallel_abort('ice_mevp: delta<=0')
              m_uvel0(i,n)=(rx*tmp1+ry*tmp2)/delta
              m_vvel0(i,n)=(-rx*tmp2+ry*tmp1)/delta
              if(ice_tests==1) then
                m_uvel0(i,n) = 1
                m_vvel0(i,n) = 0
              endif
              ...
              !endif
            enddo !n=1,ncat
            U_ice(i)=dot_product(a_icen0(i,:),m_uvel0(i,:))
            V_ice(i)=dot_product(a_icen0(i,:),m_vvel0(i,:))
          endif
        enddo !i=1,np
    ...
    do i = 1,np
      do n=1,ncat
        if(sqrt(m_uvel0(i,n)**2+m_vvel0(i,n)**2)>2) then
          tmp1 = sqrt(m_uvel0(i,n)**2+m_vvel0(i,n)**2)
          m_uvel0(i,n) = m_uvel0(i,n) * 2 / tmp1
          m_vvel0(i,n) = m_vvel0(i,n) * 2 / tmp1
        endif
      enddo
      U_ice(i)=dot_product(a_icen0(i,:),m_uvel0(i,:))
      V_ice(i)=dot_product(a_icen0(i,:),m_vvel0(i,:))
      ...
      !    do n=1,ncat
      !     m_uvel0(i,n) = m_uvel0(i,n) * 2 / sqrt(tmp1**2+tmp2**2)
      !     m_vvel0(i,n) = m_vvel0(i,n) * 2 / sqrt(tmp1**2+tmp2**2)
      !    enddo
      ! endif
      !if(isbnd(1,i)/=0) then !b.c. (including open)
      !    U_ice(i)=0; V_ice(i)=0
    ...
    do i=1,nea
      if(sigma11(i)/=sigma11(i).or.sigma12(i)/=sigma12(i).or.sigma22(i)/=sigma22(i)) then
        ...
        write(errmsg,*)'NaN in mice_mmevp (1):',ielg(i),sigma11(i),sigma12(i),sigma22(i)
        call parallel_abort(errmsg)
      endif
    enddo !i
    do i=1,npa
      if(U_ice(i)/=U_ice(i).or.V_ice(i)/=V_ice(i)) then
        write(errmsg,*)'NaN in mice_mmevp (2):',iplg(i),U_ice(i),V_ice(i)
        call parallel_abort(errmsg)
      endif
      do n=1,ncat
        if(m_uvel0(i,n)/=m_uvel0(i,n).or.m_vvel0(i,n)/=m_vvel0(i,n)) then
          write(errmsg,*)'NaN in mice_mmevp (3):',iplg(i),n,m_uvel0(i,n),m_vvel0(i,n)
          call parallel_abort(errmsg)
        endif        
      enddo
    enddo !i
