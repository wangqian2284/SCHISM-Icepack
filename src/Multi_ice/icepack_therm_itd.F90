...
!
      ...
      real (kind=dbl_kind), dimension(:), intent(inout) :: &
         hocnn,          & ! net heat flux to ocean for each category
         frzmltn
...
!          Lettie Roach, NIWA/VUW
!
      subroutine add_new_ice2 (ncat,      nilyr,      &
                              nfsd,      nblyr,      &
                              n_aero,    dt,         &
                              ntrcr,     nltrcr,     &
                              hin_max,   ktherm,     &
...
!         afsd      , & ! fsd tracer for each thickness category
      ...
      character(len=*),parameter :: subname='(add_new_ice2)'
      ...
      ! Compute the volume, area, and thickness of new ice.
      !-----------------------------------------------------------------
      !vi0new only for open water(aice0)
      !vi0newn for each ice bottom
      fnew = max (frzmltn(1)*aice0, c0)    ! fnew > 0 iff frzmlt > 0
      vi0new = -fnew*dt / qi0new ! note sign convention, qi < 0
      vi0_init = vi0new          ! for bgc

      do n=1,ncat
         fnewn(n) = max (frzmltn(n+1)*aicen(n), c0)    ! fnew > 0 iff frzmlt > 0
         vi0newn(n) = -fnewn(n)*dt / qi0new ! note sign convention, qi < 0
      enddo
      ...
      if (conserv_check) then
         ...
         eice_init = eice_init + vi0new*qi0new
         do n=1,ncat
            vice_init = vice_init + vi0newn(n)
            eice_init = eice_init + vi0newn(n)
         enddo
      endif
      ...
      do n=1,ncat
         if (update_ocn_f) then
            ...
            if (ktherm == 2) then ! return mushy-layer frazil to ocean (POP)
               vi0tmp = fnewn(n)*dt / (rhoi*Lfresh)
               dfresh = -rhoi*(vi0newn(n)- vi0tmp)/dt
               if (saltflux_option == 'prognostic') then
                  dfsalt = Si0new*p001*dfresh
               else
                  dfsalt = ice_ref_salinity*p001*dfresh
        ...
        if (tr_fsd) then ! lateral growth of existing ice
         ...
         if (ai0mod > puny) then
            ...
            if (hi0new > hi0max .and. ai0mod+puny < c1) then
               ...
               hsurp  = vsurp / aice
               vi0new = ai0new*hi0new
               !hocnn(1) = hocnn(1) + vi0new * qi0new/dt
            else
               ! put ice in a single category, with hsurp = 0
               ai0new = vi0new/hi0new
            endif
         ...
         !do n = 1, ncat
         !   hocnn(1) = hocnn(1) - vin0new(n) * qi0new/dt ! qi < 0
         !enddo
      endif                  ! vi0new > puny
      ...
      hsurpn = c0
   do n = 1, ncat
      !hsurpn(n) = min(vi0newn(n) / aicen(n), -1000 *dt / qi0new)
      !hsurpn(n) = -fnewn(n) * dt / qi0new
      if(aicen(n)>puny) hsurpn(n) = vi0newn(n) / aicen(n)
      if (hsurpn(n) > c0 .and. aicen(n)>puny) then   ! add ice to all categories
            !write(12,*) 'hsurpn',hsurpn(n),aicen(n),vicen(n),fnewn(n),frzmltn(n+1)
            vsurpn(n) = hsurpn(n) * aicen(n)
            !hocnn(n+1) = hocnn(n+1) - qi0new * vsurpn(n) / dt  ! qi < 0
            hocnn(n+1) = hocnn(n+1) - qi0new * hsurpn(n) / dt  ! qi < 0

            ! update ice age due to freezing (new ice age = dt)
            vtmp = vicen(n) + vsurpn(n)
            if (tr_iage .and. vtmp > puny) &
                trcrn(nt_iage,n) = &
               (trcrn(nt_iage,n)*vicen(n) + dt*vsurpn(n)) / vtmp

            if (tr_lvl .and. vicen(n) > puny) then
                trcrn(nt_vlvl,n) = &
               (trcrn(nt_vlvl,n)*vicen(n) + &
                trcrn(nt_alvl,n)*vsurpn(n)) / vtmp
            endif
           ...
           if (tr_iso .and. vtmp > puny) then
             do it=1,n_iso
               ...
               trcrn(nt_isoice+it-1,n)  &
                   = (trcrn(nt_isoice+it-1,n)*vicen(n) &
                   + frazil_conc*rhoi*vsurpn(n)) &
                   / vtmp

               fiso_ocn(it) = fiso_ocn(it) &
                            - frazil_conc*rhoi*vsurpn(n)/dt
             enddo
            endif
            ...
            if (ktherm == 2) then
               vsurpn(n) = hsurpn(n) * aicen(n)  ! note - save this above?
               vtmp = vicen(n) - vsurpn(n)   ! vicen is the new volume
               if (vicen(n) > c0) then
                  call update_vertical_tracers(nilyr, &
                              trcrn(nt_qice:nt_qice+nilyr-1,n), &
                              vtmp, vicen(n), qi0new)
               ...
               do k = 1, nilyr
                  ...
                  if (vicen(n) > c0) then
                     ! enthalpy
                     trcrn(nt_qice+k-1,n) = &
                    (trcrn(nt_qice+k-1,n)*vtmp + qi0new*vsurpn(n)) / vicen(n)
                     ! salinity
                     trcrn(nt_sice+k-1,n) = &
                    (trcrn(nt_sice+k-1,n)*vtmp + Sprofile(k)*vsurpn(n)) / vicen(n)
                  endif
               enddo               ! k
            endif                  ! ktherm
      endif ! hsurp > 0
   enddo                     ! n
      ...
      if (d_an_tot(n) > c0 .and. vin0new(n) > c0) then  ! add ice to category n
         ...
         tmpfrz = tmpfrz - vin0new(n) * qi0new/dt
         !hocnn(1) = hocnn(1) - vin0new(n) * qi0new/dt ! qi < 0
         !hocnn(1) = hocnn(1) - vin0new(n)/d_an_tot(n) * qi0new/dt ! qi < 0
