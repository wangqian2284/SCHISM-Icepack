...
!#include "../associate_mesh.h"
          ...
          do i=1, io_listsize
             ...
             case ('vsno      ')
                  ...
                  !call writeout_nc(id_out_var(noutput+25), 'volume_per_unit_area_of_snow',1,1,npa,wave_spectrum(:,20))
             ...
             case ('uvel      ')
                  ...
                  !call writeout_nc(id_out_var(noutput+26), 'x-component_of_ice_velocity',1,1,npa,sst)
                  call writeout_nc(id_out_var(noutput+26), 'x-component_of_ice_velocity',1,1,npa,sstnfsd_rad)
             ...
             case ('vvel      ')
                  ...
                  call writeout_nc(id_out_var(noutput+27), 'y-component_of_ice_velocity',1,1,npa,sstnbeta)
             ...
             case ('Tsfcn     ')
               do k = 1,ncat+1
                  ...
                  call writeout_nc(id_out_var(noutput+28+k), trim(longname),1,1,npa,sstn(:,k))
             ...
             case ('pond_topon')
                if (tr_pond_topo) then
                  ...
                  do k = 1,ncat+1
                     ...
                     write(longname,'(A31,i1)') 'sea ice old surf. temperature: ', k
                     call writeout_nc(id_out_var(noutput+108+k), trim(longname),1,1,npa,sstn_old(:,k))
                  enddo
             ...
             case ('pond_lvln ')
                if (tr_pond_lvl) then
                  do k = 1,ncat
                     ...
                     call writeout_nc(id_out_var(noutput+128+k), trim(longname),1,1,npa, m_uvel(:,k))
                  enddo
                  !call def_stream3D((/nod2D, ncat/),  (/nx_nh, ncat/),  'apnd', 'melt pond area fraction',          'none', trcrn(:,nt_apnd,:), io_list_icepack(i)%freq, io_list_icepack(i)%unit, io_list_icepack(i)%precision, mesh)
                  do k = 1,ncat
                     write(longname,'(A15,i1)') 'melt pond depth', k 
                     !call writeout_nc(id_out_var(noutput+138+k), trim(longname),1,1,npa, trcrn(:,nt_hpnd,k))
                     call writeout_nc(id_out_var(noutput+138+k), trim(longname),1,1,npa, m_vvel(:,k))
                  enddo
                  !call def_stream3D((/nod2D, ncat/),  (/nx_nh, ncat/),  'hpnd', 'melt pond depth',                  'm',    trcrn(:,nt_hpnd,:), io_list_icepack(i)%freq, io_list_icepack(i)%unit, io_list_icepack(i)%precision, mesh)
                  do k = 1,ncat
    ...
    module subroutine restart_icepack(ncid_hot,nvars_hot,node_dim)
        ...
        integer (kind=int_kind)   :: i, j, k, iblk, &     ! counting indices
                                     ...
                                     var2d2_dim(2),ice_ntr_dim,ice_ntr_dim2,nvars_hot0
...
!#include "../associate_mesh.h"
        ...
        j=nf90_def_var(ncid_hot,'sstn_old',NF90_DOUBLE,var2d2_dim,nwild(nvars_hot0+5))
        j=nf90_def_var(ncid_hot,'hocnn_old',NF90_DOUBLE,var2d2_dim,nwild(nvars_hot0+6))
        j=nf90_def_var(ncid_hot,'fswthrun_old',NF90_DOUBLE,var2d2_dim,nwild(nvars_hot0+7))
        nvars_hot0=nvars_hot0+7
        if (tr_iage) then
     ...
     if (tr_snow) then
      ...
      if(tr_fsd) then
        ...
        swild3 = transpose(sstn_old) 
        j=nf90_put_var(ncid_hot,nwild(nvars_hot+5),swild3,(/1,1/),(/ncat+1,np/))
        swild3 = transpose(hocnn_old) 
        j=nf90_put_var(ncid_hot,nwild(nvars_hot+6),swild3,(/1,1/),(/ncat+1,np/))
        swild3 = transpose(fswthrun_old) 
        j=nf90_put_var(ncid_hot,nwild(nvars_hot+7),swild3,(/1,1/),(/ncat+1,np/))
