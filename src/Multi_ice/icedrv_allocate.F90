...
! -------------------------------------------------------------
  ...
  submodule (icedrv_main) allocate_icepack 
      ...
      allocate (               &
         ...
         m_uvel    (nx,ncat) , & ! x-component of velocity (m/s) on the nodes for each category
         m_vvel    (nx,ncat) , & ! y-component of velocity (m/s) on the nodes for each category
      ...
      allocate( &
         ...
         strocnxTn (nx,ncat) , & ! ice-ocean stress for each category, x-direction
         strocnyTn (nx,ncat) , & ! ice-ocean stress for each category, y-direction
         ...
         sstn(nx,ncat+1),&! sea surface temperature (C) for each ice category and open water
         sstn_old(nx,ncat+1),&! sea surface temperature (C) for each ice category and open water, old
         sstnbeta(nx)   ,& ! mixing term for sstn
         sstnfsd_rad(nx),& ! mean radius of floe size
         nfloe(nx)      ,& ! number of floe
         frzmlt(nx)  , & ! freezing/melting potential (W/m^2)
         frzmltn(nx,ncat+1),& !freezing/melting potential (W/m^2) for each category
         ...
         hocnn(nx,ncat+1), & ! net heat fulx of each ice categoryto ocean (W/m^2)
         hocnn_old(nx,ncat+1),& ! net heat fulx of each ice categoryto ocean (W/m^2)
         ...
         fbotn(nx,ncat),&!heat flux at bottom surface of ice (excluding excess) for each category (W/m^2)
      ...
      allocate(             &
         ...
         fswthrun_old(nx,ncat+1)       , & ! SW through ice to ocean (old)      (W/m^2)
